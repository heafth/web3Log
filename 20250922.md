...
/* type Cat struct { animal Animal }，和以前 type Cat struct { Animal }（匿名字段）是有区别的：

匿名字段（type Cat struct { Animal }）：
Cat 自动拥有 Animal 的所有字段和方法，可以直接用 cat.name 访问 name 字段。
语法糖，类似“继承”。
普通字段（type Cat struct { animal Animal }）：
Cat 只是有一个名为 animal 的字段，类型是 Animal。
访问 name 字段时，必须写 cat.animal.name，不能直接写 cat.name。 */
...
/*
转成 JSON 后，字段名就会变成 "nickname"（小写），
而不是 Go 里的 "Nickname"（大写）
这种写法`json:"-"` 表示转json的时候隐藏
加上,omitempty 表示为0则不显示
*/
type Student struct {
...
func main() {
	student := Student{Name: "豆豆", Age: 0, Nickname: "外号", Password: "Password"}
	fmt.Print(student)
	byteData, _ := json.Marshal(student)

	//强转成字符串string(byteData)
